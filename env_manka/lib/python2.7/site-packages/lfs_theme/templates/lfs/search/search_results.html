...
{% block title %}
    {% trans 'Результаты поиска' %} - {{ block.super }}
{% endblock %}

{% block section %}search-results{% endblock %}

{% block breadcrumbs %}
    {% breadcrumbs _("Результаты поиска") %}
{% endblock %}

{% block wrapper %}
    <h1 class="first-heading">{% trans 'Результаты поиска' %}</h1>

    <label for="search">{% trans 'Что ищем?' %}:</label>
    ...
    <p class="search-results-description">
        {% trans 'Показаны результаты по запросу:' %} <span class="search-results-query">"{{ q }}"</span>.
        {% if total == 0 %}
